...
def genoff_tsn(model, mname, batch_size, core_number, in_height, in_width,
           half_input, input_format, fake_device, dtype):
    # set offline flag
    ...
    model_online_fullname = pj(model_path, 'mlu_mimo_{}_{}_{:d}x{:d}.pth'.format(dtype, model,in_width, in_height))
    ...
    use_firstconv = True
    loading = False
    net = SmokingModel_1_MIMO_MLU()
    print('==end==')
    net = net.eval().float()

    net = mlu_quantize.quantize_dynamic_mlu(net, dtype=dtype)
    ...
    randn_mlu = torch.randn(1, T, args.channel_size, IMG_SIZE[1], IMG_SIZE[0], dtype=torch.float)
    # if use_firstconv:
    example_mlu = example_mlu*255
    randn_mlu = randn_mlu*255

    print(example_mlu.shape)

    if half_input:
        randn_mlu = randn_mlu.type(torch.HalfTensor)
        example_mlu = example_mlu.type(torch.HalfTensor)

    randn_mlu = [ randn_mlu[:,i,...].to(ct.mlu_device()) for i in range(T) ]
    example_mlu = [example_mlu[:,i,...].to(ct.mlu_device()) for i in range(T)]
    ...
    net_traced(example_mlu[0], example_mlu[1], example_mlu[2])
...
if __name__ == "__main__":
    ...
    if quantized_mode == 2:
        dtype = 'int16'
    elif quantized_mode == 1:
        dtype = 'int8'
    elif quantized_mode == 0:
        dtype = 'float'
    ...
    if autotune:
        mname = './{}/{}_mimo_auto_{}x{}_{}_t{}bs{}c{}{}_{}'.format(platform_name, model, in_width, in_height, platform_name, args.t ,batch_size, core_number, fpX, dtype)
    else:
        mname = './{}/{}_mimo_{}x{}_{}_t{}bs{}c{}{}_{}'.format(platform_name, model, in_width, in_height, platform_name, args.t ,batch_size, core_number, fpX, dtype)
    ...
    genoff_tsn(model, mname, batch_size, core_number,
           in_height, in_width, half_input, input_format, fake_device, dtype)