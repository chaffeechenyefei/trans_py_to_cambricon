...
def preprocess_b_c_h_w(img_cv, dst_size ,mean, std, use_rgb = True,mlu=False):
    """
    :param img_cv: 
    :param dst_size: 
    :param mean: 
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--mpath',default='pth/track_r18/milestone')
    ...
    #use init weights
    parser.add_argument("--fake", action='store_true', help="use random weights for test")
    ...
    args_w = config_param[config_key]['w']
    args_h = config_param[config_key]['h']
    ...
    model_online_fullname = pj(model_path, 'mlu_{}_{}_{}_{:d}x{:d}.pth'.format(dtype,model_framework,model_date,
                                                                               args_w,args_h))
    IMG_SIZE = [args_w,args_h]  # [w,h]
    ...
    [c,h,w]xN
    """
    data = [preprocess_b_c_h_w(c , dst_size=IMG_SIZE , mean=py_mean, std = py_std, use_rgb=config_param[config_key]['rgb'] ,mlu=flag_preprocess_mlu) for c in input_img]
    ...
    if args.fake:
        ...
        if hasattr(model, "init_weights"):
            ...
            print('saving ',pj(save_dir, model_name))
            checkpoint_op.save_checkpoint(model=model, ckpt_fpath=model_name)
        else:
            print('ERROR::model.init_weights() is not initialed!!!')
            exit(-1)
    ...
    if args.quantization:
        ...
        flag_per_channel = True
        if 'quant_per_channel' in config_param[config_key].keys():
            flag_per_channel = config_param[config_key]['quant_per_channel']
        print('quant_per_channel:', flag_per_channel)
        qconfig = { 'per_channel':False, 'firstconv':use_first_conv ,'mean': mlu_mean , 'std': mlu_std}
