...
class PoseResNet(nn.Module):
    ...
    def _make_deconv_layer(self, num_layers, num_filters, num_kernels):
        ...
        for i in range(num_layers):
            ...
            print('ic:', self.inplanes, ' oc:', planes, ' k:', kernel, ' s:', 2, ' p:', padding, ' op:', output_padding, ' b:', self.deconv_with_bias)
...
def pose_resnet18():
    model = PoseResNet(block=BasicBlock,
                       layers=[2, 2, 2, 2],
                       cfg=EasyDict(
                           dict(
                               EXTRA=dict(
...
class PoseNet_Model_MLU(nn.Module):
    def __init__(self):
        super(PoseNet_Model_MLU, self).__init__()
        self.backbone = pose_resnet18()
...
if __name__ == '__main__':
    ...
    x = torch.randn(2, 3, 256, 192, dtype=torch.float32)
    y = model(x)
    print(y.shape)
