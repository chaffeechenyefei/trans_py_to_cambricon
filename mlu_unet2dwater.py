...
def preprocess_b_c_h_w(img_cv, dst_size ,mean, std, mlu=False):
    """
    :param img_cv: 
    :param dst_size: 
    :param mean: 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mpath',default='pth/UNet2D_Water/milestone')
    # Check
    ...
    model_name = 'model_{}_no_serial.pth'.format(model_key)#old format unzipped
    print('@@Model pth = {}'.format(model_name))
    model_online_fullname = pj(model_path, 'mlu_{}_{}_{}_{:d}x{:d}.pth'.format(dtype,model_framework,model_key,
                                                                               args.w,args.h))
    ...
    [c,h,w]xN
    """
    data = [preprocess_b_c_h_w(c , dst_size=IMG_SIZE , mean=py_mean, std = py_std, mlu=flag_preprocess_mlu) for c in input_img]
