...
class HopenetMBV2(nn.Module):
    ...
    def __init__(self, num_bins=60):
        ...
        self.wbins = nn.Parameter(torch.FloatTensor(np.arange(num_bins)).reshape(1,1,-1),False)#[1,1,60]
    ...
    def forward(self, x):
        ...
        output = torch.sum(output*self.wbins, dim=-1, keepdim=False) #[b,3,bins]->[b,3]
        output = output*3 - 90
        print(output.shape)
        return output
